...
///// САМЫЙ ПРАВИЛЬНЫЙ СПОСОБ ДЛЯ НАЗНАЧЕНИЯ ОБРАБОТЧИКА НА НЕСКОЛЬКО ЭЛЕМЕНТОВ
/// повесить событие на сразу несколько кнопок
btn.forEach(btn => { // btn - каждая отдельная кнопка, находящаяся внутри
    btn.addEventListener('click', deleteElement);
});
...
/// ОПЦИИ СОБЫТИЯ (их много)
// выполнить всего раз
btn.forEach(btn => { 
    btn.addEventListener('click', deleteElement, {once:true}); // выполнится всего раз, потому что сработала опция once
});